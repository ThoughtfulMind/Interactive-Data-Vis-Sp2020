class Table {
  ...
  constructor(state, setGlobalState) {
    ...
    const slimmedData = country.data.map(d => ({
      "Country": d.Country,
        "Country",
      "Beds": d['Beds']
    })).sort((a, b) => d3.descending(a['Beds'], b['Beds']))
    ...
    const logScale = d3
      ...
      .domain(d3.extent(slimmedData, d => d['Beds']))
    ...
    this.tableRows = table
      ...
      .data(slimmedData)
      .join("tr")
      .style("background-color", d => this.colorScale(d['Beds']))
      .style("color", "#eee");
  ...
  draw(state, setGlobalState) {
    ...
    this.tableRows.style("background-color", d =>
      state.selectedState === d.Country ? "grey" : this.colorScale(d['Beds'])
    );
  }
}
